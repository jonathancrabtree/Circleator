package Circleator::FeatFunction::Color::snp_type;
...
sub get_function {
  ...
  my $ins_color = $track->{'snp-ins-color'} || '#4b0082'; # indigo
  my $del_color = $track->{'snp-del-color'} || '#455b1f'; # darkolivegreen
  ...
  return sub {
    ...
    my $num_insertions = 0;        # number of alleles that are insertions
    my $num_deletions = 0;         # number of alleles that are deletions
    ...
    foreach my $qb (@q_bases) {
      ...
      my $ql = length($qb);
      # insertion - ref is "." or "-"
      ++$num_insertions if (($ql > $rl) || ($r_bases[0] =~ /^[\.\-]+$/));
      # deletion - query is "." or "-"
      ++$num_deletions if (($ql < $rl) || ($q_bases[0] =~ /^[\.\-]+$/));
    }
    
    return $no_hit_color if ($no_hit);
    return $same_as_ref_color if ($same_as_ref);
    return $ins_color if ($num_insertions == $num_qb);
    return $del_color if ($num_deletions == $num_qb);
