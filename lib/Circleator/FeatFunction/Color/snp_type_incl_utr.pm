package Circleator::FeatFunction::Color::snp_type_incl_utr;
...
sub get_function {
  ...
  my $utr_color = $track->{'snp-utr-color'} || 'purple';
  ...
  return sub {
    ...
    if ($f->has_tag($SNP_TP . 'syn_nonsyn')) {
      ...
      # HACK - this is specific to EUK-501
      if ($sns_vals[0] =~ /^Uncertain|Mut$/i) {
